...
layout(location =  4) uniform float size_x;
layout(location =  5) uniform float size_y;
layout(location =  6) uniform float size_z;
layout(location =  7) uniform float perlin_z;
...
vec4 perlin_color() {
    float r = perlin(vec3(scaled_uvs * size_x, perlin_z));
    float g = perlin(vec3(scaled_uvs * size_y, perlin_z));
    float b = perlin(vec3(scaled_uvs * size_z, perlin_z));
...
void main() {
    vec4 color = texture(tex, scaled_uvs);
    vec4 actual = perlin_color();
